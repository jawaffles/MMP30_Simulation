...
geometry_msgs::PointStamped UTMtoMapPoint(geometry_msgs::PointStamped UTM_input)
{
    ...
    while(notDone)
    {
        try
        {
            ...
            listener.waitForTransform("utm_relative", "utm", time_now, ros::Duration(3.0));
            tf::StampedTransform transform_map;
            listener.lookupTransform("utm_relative", "utm", ros::Time(0), transform_map);
            map_point_output.header.frame_id="utm_relative";
            ...
            ROS_INFO("%f, %f",origin_x, origin_y);
            //listener.transformPoint("map", UTM_input, map_point_output);
            map_point_output.point.x=UTM_input.point.x-origin_x;
            map_point_output.point.y=UTM_input.point.y-origin_y;
            notDone = false;
        }
