...
std::vector <std::pair<double, double>> getWaypoints(std::string path_local)
{
  ...
  ROS_INFO("The following Odom Waypoints have been set:");
...
geometry_msgs::PointStamped latlongtoUTM(double lati_input, double longi_input)
{
 ...
 utm_y = longi_input;
  ...
  UTM_point_output.header.frame_id = "odom";
...
int main(int argc, char** argv)
{
  ...
  std::string path_local = "/odom_coordinates_files.txt" ;
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
        {
            ...
            ROS_INFO( " Odom: %f, %f" ,UTM_point.point.x,UTM_point.point.y );


            //Build goal to send to move_base //initiate a move_base_msg called goal
            move_base_msgs::MoveBaseGoal goal = buildGoal(UTM_point, UTM_next, final_point);
