...
geometry_msgs::PointStamped latlongtoUTM(double lati_input, double longi_input)
{
  ...
  UTM_point_output.header.frame_id = "beacon_map";
...
geometry_msgs::PointStamped UTMtoMapPoint(geometry_msgs::PointStamped UTM_input)
{
    ...
    while(notDone)
    {
        try
        {
            ...
            listener.waitForTransform("odom", "beacon_map", time_now, ros::Duration(3.0));
...

{
  ...
  std::string path_local = "/indoor_coordinates_files.txt" ;
