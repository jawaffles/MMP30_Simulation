...
def pcd_row_detection(pcd):
        ...
        #Desired Heading + final Plot
                plt.plot([line_X_r[0],line_X_r[2]], [0,0], color='black', linewidth=lw, label='Rover Heading')           
...
def row_angle_server():
    rospy.init_node('row_angle_server')
    s = rospy.Service('row_angle', row_angle, handle_row_angle)
    print "Ready to calculate row angle"
    rospy.spin()

def handle_row_angle(req):
    print("Point cloud has {} points").format(len(req.pcdin.data))
    return row_angleResponse(len(req.pcdin.data))
