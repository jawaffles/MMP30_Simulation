...
class MoveBaseActionServer(object):
	...
	def __init__(self, name):
		...
		self.__feedback = MoveBaseFeedback()
		...
		self.__odometrySub = rospy.Subscriber(
			"/odom", Odometry, self.__odometryCallback, queue_size=1)
		self.__simpleGoalSub = rospy.Subscriber(
			"move_base_simple/goal", PoseStamped, self.__simpleGaolCallback, queue_size=1)
		self.__cmdVelPub = rospy.Publisher("cmd_vel", Twist, queue_size=1)
		self.__action_goal_pub = rospy.Publisher(
			"move_base/goal", MoveBaseActionGoal, queue_size=1)
		self.__targetPointPub = rospy.Publisher(
			"move_base/target_point", PointStamped, queue_size=1)
		self.__pathPub = rospy.Publisher(
			"move_base/path", Path, queue_size=1, latch=True)
		...
		self.__min_in_place_vel_theta = float(
		    rospy.get_param('~min_in_place_vel_theta', 0.04))
		self.__yaw_goal_tolerance = float(
		    rospy.get_param('~yaw_goal_tolerance', 0.05))
		...
		self.__controller_frequency = float(
		    rospy.get_param('~controller_frequency', 20))
		self.__dt = 1.0/self.__controller_frequency
		self.__transform_tolerance = rospy.Duration(
		    float(rospy.get_param('~transform_tolerance', 1)))
	...
	def __simpleGaolCallback(self, goal):
		...
		self.__action_goal_pub.publish(action_goal)
	...
	def __execute(self, goal):
		...
		try:
			self.__tfListener.waitForTransform(
			    goal_frame, self.__robot_base_frame, now, self.__transform_tolerance)
		except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException, tf.Exception) as exp:
			rospy.logwarn("%s" % exp)
			success = False
			self.__as.set_aborted()
		...
		endPose=goal.target_pose
		endPosition=goal.target_pose.pose.position
		q = (goal.target_pose.pose.orientation.x, goal.target_pose.pose.orientation.y, goal.target_pose.pose.orientation.z, goal.target_pose.pose.orientation.w)
		...
		while (True):
			...
			except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException, tf.LookupException, tf.Exception) as exp:
				rospy.logwarn("%s" % exp)
				# self.__feedback.status.status=GoalStatus.ABORTED
			...
			if state==START or state==END:
				...
				rospy.logdebug ("heading error is %f" % heading_error)	
				...
				if abs(heading_error)>self.__yaw_goal_tolerance:
					...
					if abs(rotateAcc) > self.__acc_lim_theta:
						in_place_rotation_vel=(in_place_rotation_vel+self.__acc_lim_theta*self.__dt)*(-1 if new_in_place_rotation_vel<0 else 1)
					else:
			...
			# perform the pure pursuit
				...
				if distToGoal2 < self.__xy_goal_tolerance*self.__xy_goal_tolerance:
					...
					self.__feedback.base_position=currentRobotPose
